...
and processing of only the required data in a single gdalwarp call. This reduces
...
for Sentinel-2 data, create a vrt_collection object (basically a list of warped 
VRTs). We then apply the masking using pixel functions (I can't get internal or 
external masks to work for these VRTs), finally these images are stacked 
(combined into a single VRT with multiple layers in each VRTRasterBand), the
median pixel function is then added to the VRT and all of this is then "lazily"
calculated at the end of the vrt pipeline using gdalwarp.
