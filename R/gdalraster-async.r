...
mirai_async_result_handler <- function(
  ...
  promise_list <- purrr::imap(jobs, function(j = .x, i = .y) {
    j |>
      promises::then(
        ...
        onRejected = function(e) {
          completed_count <<- completed_count + 1
          cli::cli_abort("promise rejected: {e}", class = "mirai_async_error")
        }
      ) |>
      promises::catch(function(e) {
        completed_count <<- completed_count + 1
        cli::cli_abort("promise catch: {e}", class = "mirai_async_error")
      })
  })
  ...
  # Wait for mirai jobs to complete
  mirai::collect_mirai(jobs)

  # CRITICAL: Pump the event loop to process promise callbacks
