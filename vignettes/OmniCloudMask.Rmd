...
necessary, but in this case, we need the data on disk because cloud mask 
inference cannot be run in parallel (at least not on my laptop). We use 
...
Once the mask is created, it will be added as a new band to the VRT object. 
We can then compute the mask with `vrt_compute()`, which will then materialise 
the mask as a new raster file. Note that we use `recollect = TRUE` to return a 
new `vrt_collection` object because we plan to do some further processing.
