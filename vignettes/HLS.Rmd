...
case because of the multiple SRS in the collection. Also to make use of the
...
easily by first using the following:


``` r
band_level_median <- vrt_stack(hls_warp) |>
...
if desired. Here we will use the `geomedoid` function which is a  somewhat
experimental function that combines the geometric median and medoid. The 
geometric median is calculated and used to locate the image cell that has the
smallest distance to the geometric median. 


``` r
hls_stack <- multiband_reduce(hls_warp, reduce_fun = geomedoid())
#> Error in `purrr::pmap()`:
#> ℹ In index: 1.
...
#> Error: object 'hls_composite' not found
```
...
#> ! object 'hls_composite' not found
```
